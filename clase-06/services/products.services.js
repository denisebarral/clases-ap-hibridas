...
/**
 * editProduct - Edita un producto existente en el archivo JSON.
 *
 * Busca el producto por id dentro del array y actualiza sus campos
 * nombre y precio. Luego sobreescribe el archivo con el array actualizado.
 *
 * Usa == (doble igual) para comparar ids sin importar el tipo,
 * ya que el id de la URL llega como string y en el JSON puede ser número.
 *
 * @param {string|number} id      - El ID del producto a editar.
 * @param {Object}        producto - Objeto con los campos a actualizar ({ nombre, precio }).
 * @returns {Promise<Object>} Promise que resuelve con el objeto producto editado.
 * @throws {Error} Si el archivo no existe o no se puede escribir.
 */
export async function editProduct(id, producto) {
    ...
    try {
        ...
        // 2. Recorre el array y actualiza nombre y precio del producto que coincide con el id
        ...
        // 3. Verifica que el archivo exista antes de sobreescribir
        await access(archivo, constants.F_OK)
        // 4. Sobreescribe el archivo con el array actualizado
        await writeFile(archivo, JSON.stringify(productos))
        // 5. Devuelve el objeto con los datos editados
        return producto
    } catch (error) {
        throw new Error(error)
    }
...
/**
 * deleteProduct - Marca un producto como borrado en el archivo JSON (soft delete).
 *
 * En lugar de eliminar el objeto del array, le agrega la propiedad borrado: true.
 * Esto conserva el historial y evita reutilizar IDs de productos eliminados.
 *
 * Pasos:
 *  1. Obtiene todos los productos.
 *  2. Busca el que coincide con el id y le agrega borrado: true.
 *  3. Guarda el array actualizado en el archivo.
 *  4. Devuelve el objeto del producto que fue marcado como borrado.
 *
 * @param {string|number} id - El ID del producto a eliminar.
 * @returns {Promise<Object>} Promise que resuelve con el producto marcado como borrado.
 * @throws {Error} Si el archivo no existe o no se puede escribir.
 */
export async function deleteProduct(id) {
    try {
        ...
        // 2. Busca el producto por id, lo guarda en productoBorrado y le agrega borrado: true
        productos.forEach((producto) => {
            if (producto.id == id) {
                productoBorrado = producto
                // Soft delete: no se elimina del array, se marca como borrado
        ...
        // 3. Verifica que el archivo exista antes de sobreescribir
        await access(archivo, constants.F_OK)
        // 4. Sobreescribe el archivo con el array actualizado (producto marcado como borrado)
        await writeFile(archivo, JSON.stringify(productos))
        // 5. Devuelve el producto que fue marcado como borrado
